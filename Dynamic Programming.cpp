...
// 2-d dp unique paths 
Whenever someone says all possible ways use recursion 
Steps to write the recurrence relation for a 2d dp:
1.) Try to write everything in the form of indeexes in 2-d dp in the form of i and j
2.) do all stuffs on those index (using by creating a small tree like structure chekcing the aloowed values and all in the qquestion)
3.) return the ans as asked 

to count the ways we have to write a base case we have return 1 if we reached the base case else we return 0
like specifically here we will check 

to remove overlapping subproblems we use a concept called memoization 
to use memoization we check the chngaing parametres like here are the 2d dp 2 variable i and j 

tabulation is bottom up becase we start from the base case and goes to the top accumulating the results 

In space optimization whenever we see a fetching of previous row and prevoious col we can possible do space optimiztaion 

To solve dp question well pehle recursion wala likh diya akro then dp laga diya karo 
...
public:
    ...
    int solve(int i,int j,vector<vector<int>> &dp){
        if(i == 0 && j == 0) return 1;  //base conditions 
        if (i < 0 || j < 0) return 0;
        if(dp[i][j] != -1){
            return dp[i][j];
        }
...
//Leetcode 1143
In solving dp question whnever you see like you have repeating/overlappingsubproblems then use dp and also when you have options as well also yaha optimal bola hai(like max min liek this)
class Solution {
public:
